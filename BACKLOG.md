...
0. **Verify browser reachability/CORS against real endpoints — DONE 2026-09-22.** Probed `https://litemaas.rhoai.rh-aiservices-bu.com/v1` with `curl -X OPTIONS` (`Origin` + `Access-Control-Request-Method: POST` + `Access-Control-Request-Headers: authorization, content-type`): preflight 200 from both `https://rh-aiservices-bu.github.io` and `http://localhost:3010` — uvicorn CORS middleware echoes any origin (`vary: Origin`, `allow-credentials: true`, `allow-headers: authorization, content-type`, max-age 600). Unauthenticated `GET /v1/models` returns 401 **with CORS headers** (`allow-origin: *`), so error responses are readable by the failure UX, not network-opaque. Conclusion: both primary uses work in-browser for this RHOAI route; the curl-twin fallback stays as conditional guidance in `curl.md` for endpoints without CORS middleware.
